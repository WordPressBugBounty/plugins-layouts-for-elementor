...
class Layout_For_Elementor {
    ...
    public function lfe_layouts_widget_fail_load() {
        ...
        if (isset($installed_plugins[$file_path])) { // check if plugin is installed
            ...
            $message = '<p><strong>' . __('Layouts for Elementor', 'layouts-for-elementor') . '</strong>' . __(' widgets not working because you need to activate the Elementor plugin.', 'layouts-for-elementor') . '</p>';
            $message .= '<p>' . sprintf('<a href="%s" class="button-primary">%s</a>', $activation_url, __('Activate Elementor Now', 'layouts-for-elementor')) . '</p>';
        } else {
            ...
            $message = '<p><strong>' . __('Layouts for Elementor', 'layouts-for-elementor') . '</strong>' . __(' widgets not working because you need to install the Elemenor plugin', 'layouts-for-elementor') . '</p>';
            $message .= '<p>' . sprintf('<a href="%s" class="button-primary">%s</a>', $install_url, __('Install Elementor Now', 'layouts-for-elementor')) . '</p>';
        }
    ...
    public function lfe_layouts_elementor_update_notice() {
        ...
        $message = '<p><strong>' . __('Layouts for Elementor', 'layouts-for-elementor') . '</strong>' . __(' widgets not working because you are using an old version of Elementor.', 'layouts-for-elementor') . '</p>';
        $message .= '<p>' . sprintf('<a href="%s" class="button-primary">%s</a>', $upgrade_link, __('Update Elementor Now', 'layouts-for-elementor')) . '</p>';
    ...
    public function lfe_admin_scripts() {
        ...
        wp_register_style('lfe-admin-stylesheets', LFE_URL . 'assets/css/admin.css', array(), 1.0, false);
        wp_register_style('lfe-toastify-stylesheets', LFE_URL . 'assets/css/toastify.css', array(), 1.0, false);
